...
CREATE TABLE p_characters (
    ...
    name varchar(16) CHECK (name ~ '^[A-Za-z]{3,16}$') UNIQUE NOT NULL,
    hair_color varchar(7) CHECK (hair_color ~ '^#[0-9A-Fa-f]{6}$') NOT NULL,
    hp_max integer CHECK (hp_max >= 0) NOT NULL,
    hp integer CHECK (
        hp >= 0 AND
        hp <= hp_max
    ) NOT NULL,
    ...
    height_cm numeric(4, 1) CHECK (height_cm BETWEEN 50.0 AND 250.0) NOT NULL,
    created_on timestamp NOT NULL CHECK (created_on BETWEEN '2025-12-01' AND CURRENT_TIMESTAMP),
    deleted_on timestamp CHECK (
        deleted_on IS NULL OR
        deleted_on >= created_on
    )
);
